...
def train_process(dataset, subset):
    ...
    with open(os.path.join(TARGET_MODEL_DIR, f'KMeans_{dataset}_{subset}.model'), 'wb') as f:
        pickle.dump(model, f)
...
def test_process(dataset, subset):
    with open(os.path.join(TARGET_MODEL_DIR, f'KMeans_{dataset}_{subset}.model'), 'rb') as f:
        model = pickle.load(f)
